...
export const createStructure = <
    T extends ReturnType<typeof createBrowserPath>[number] & ReturnType<typeof createImportPath>[number],
>(
    sources: T[],
) => {
    type DeepModule<P> = P & {
    ...
    sources.forEach((source) => {
        if ((source.metadata?.nested === void 0 || source.metadata?.nested === true) && source.browserPath !== '/') {
